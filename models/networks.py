...
def define_network(input_nc, lstm_hidden_size, model, init_from=None, isTest=False, f_size=14, gpu_ids=[]):
    netG = None
...
class RegressionHead(nn.Module):
    def __init__(self, lossID, weights=None, lstm_hidden_size=None, k_size=14):
        ...
        if lossID != "loss3":
            self.projection = nn.Sequential(*[nn.AvgPool2d(kernel_size=k_size-9, stride=3),
            ...
            self.projection = nn.AvgPool2d(kernel_size=ceil(k_size/2), stride=1)
...
class PoseNet(nn.Module):
    ...
    def forward(self, input):
        ...
        output_5b = self.inception_5b(output_5a) #75 1024 8 8
...
class CloudNet(nn.Module):
    def __init__(self, input_nc, lstm_hidden_size, weights=None, isTest=False, f_size=None, gpu_ids=[]):
        super(CloudNet, self).__init__()
        k_size = ceil(f_size/16)
        self.gpu_ids = gpu_ids
        self.isTest = isTest
        self.before_inception = nn.Sequential(*[
            weight_init_googlenet("conv1/7x7_s2", nn.Conv2d(input_nc, 64, kernel_size=7, stride=2, padding=3), weights),
        ...
        self.cls1_fc = RegressionHead(lossID="loss1", weights=weights, k_size=k_size)
        self.cls2_fc = RegressionHead(lossID="loss2", weights=weights, k_size=k_size)
        self.cls3_fc = RegressionHead(lossID="loss3", weights=weights, k_size=k_size)

    def forward(self, input):
        output_bf = self.before_inception(input) # batch,192,28,28
        output_3a = self.inception_3a(output_bf) # batch,256,28,28
        output_3b = self.inception_3b(output_3a) # batch,480,14,14
        output_4a = self.inception_4a(output_3b) # batch,512,14,14
        output_4b = self.inception_4b(output_4a) # batch,512,14,14
        output_4c = self.inception_4c(output_4b) # batch,512,14,14
        output_4d = self.inception_4d(output_4c) # batch,528,14,14
        output_4e = self.inception_4e(output_4d) # batch,832,7,7
        output_5a = self.inception_5a(output_4e) # batch,832,7,7
        output_5b = self.inception_5b(output_5a) # batch,1024,7,7
        return self.cls1_fc(output_4a) + self.cls2_fc(output_4d) + self.cls3_fc(output_5b)
