...
def define_network(input_nc, lstm_hidden_size, model, init_from=None, isTest=False, gpu_ids=[]):
    ...
    if model == 'posenet':
        ...
        netG = CloudNet(input_nc, lstm_hidden_size, weights=init_from, isTest=isTest, gpu_ids=gpu_ids)
    if len(gpu_ids) > 0:
        netG.cuda(gpu_ids[0])
    return netG
...
class CloudNet(PoseNet):
    def __init__(self, input_nc, lstm_hidden_size, weights=None, isTest=False,  gpu_ids=[]):
        super(CloudNet, self).__init__(input_nc, weights, isTest, gpu_ids)
